...
// ----------------------------------------------------------------------------   
  ...
  return db.species.find(criteria, { order: [{field: 'species_code_nznbbs', direction: 'asc'}]});
}
...
// ----------------------------------------------------------------------------
  ...
  return BoilerPlate.getSchemaFromDynamo(parameterSchemaParams.table, parameterSchemaParams.id, parameterSchemaParams.version)
    ...
    .then(dbAccess => {
      console.info('Getting species');
      db = dbAccess.db;
      dbCreationTimestamp = dbAccess.dbCreationTimestamp;
      return search(db, event.pathParameters.speciesId); 
    })  
    .then(res => {
      if (res.length === 0) throw new BoilerPlate.NotFoundError('SpeciesNotFound', { type: 'NOT_FOUND', message: `Species cannot be found with this id`, data: { path: `pathParameters.speciesId` }, value: event.pathParameters.speciesId , schema: null });
...
// ----------------------------------------------------------------------------
  ...
  return BoilerPlate.getSchemaFromDynamo(parameterSchemaParams.table, parameterSchemaParams.id, parameterSchemaParams.version)
    ...
    .then(dbAccess => {
      console.info('Searching species');
      db = dbAccess.db;
      dbCreationTimestamp = dbAccess.dbCreationTimestamp;
      return search(db); 
    })
